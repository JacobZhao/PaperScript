...
The work in \cite{nguyen2008adaptive} proposed a switch matrix to reconfigure shadowed PV array, but it can not be able to apply on large size PV array. Paper \cite{storey2013improved} using ``irradiance level equation'' method to dynamic reconfigure PV array and improve power generation efficiency over 10\%.
% based on previous studies we propose an algorithm that can reconfigure PV array with low computational expense and high feasibility.

In this paper, we use un-uniform irradiance distribution to represent the mismatch condition that PV array experienced. 
