...
\begin{algorithm}[htbp]
  ...
  \Input{
    \textit{$Q = (Q_{1}, Q_{2}, \ldots ,Q_{s}$)}: current sequence for given configuration; \\
    \textit{$m$}: the number of required active modules per-string;\\
    \textit{$M_{p}$}: the number of active modules in each panel $p$;\\
  ...
  \For{\textup{each target current level} $Q_{k}$ $(1 \leq k \leq s)$ \textup{and each panel} $p$}
  {
    \uIf{$\sum M_{p}(Q_{k})$ $<$ $m$}
    {\textup{\textbf{return :}} Feasibility = NO\\
    \tcc{Not enough active modules to meet requirement}}
    \Else{$Loss-rate(p,k,k+1) = (M_{p}(Q_{k+1}) - M_{p}(Q_{k}))/M_{p}(Q_{k})$}
  }

  \For{\textup{each} target current level $Q_{k}$}
  {
    % {\If{$\sum M_{p}(Q_{j})$ \textup{for unselected panels} $< m$}
    %   {\textup{\textbf{return :}} Feasibility = NO}}
    $P$ = a set of un-selected panels with $M_{p}(Q_{k})$;\\
     \tcp{\textbf{step~\ref{step:1}}}
     Sort $PS$ in a lexicographically ascending order of $Loss-rate$ and descending order of $M_{P}(Q_{j})$\\
     % $Loss(p,j+2)$, $\ldots$, $Loss(p,s)$, $M_{p}(Q_{j})$;\\
     \tcp{\textbf{step~\ref{step:2}}}
     select panels in the order in $PS$ until $\sum M_{p}(Q_{j})$ for selected panels $\geq m$;\\
     % \If{$\sum M_{p}(Q_{j})$ \textup{for selected panels} = $m$}
     % {\textup{\textbf{return :}} Feasibility = YES}
     \tcp{\textbf{step~\ref{step:3}}}
...
% $\dots$,$Loss-rate(p,k^{'},s)$, M$_{p}(Q_{k^{'}})$.
