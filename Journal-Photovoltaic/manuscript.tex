...
For example, as the panel $p$ in Table~\ref{tab:NumofModules} $Loss-rate(p,Q_{1},Q_{2})=0$, $Loss-rate(p,Q_{1},Q_{3})=0.5$, that is, for target current level $Q_{1}$ there is no ``$loss$'' for current level $Q_{2}$ and only loss ``0.5'' active module for current level $Q_{3}$.
