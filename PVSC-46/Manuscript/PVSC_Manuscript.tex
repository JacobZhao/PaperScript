...
However,  the operation of bypass diode will cause a number of PV cells to stop deliver power, it will introduce voltage drop which cause PV panel mismatched.
...
The maximum power points currents for these modules are 4.3A, 3.0A, and 1.5A, respectively. 
...
This algorithm is roughly divided into 4 steps.
...
To evaluate the performance of proposed algorithm, we compare the result with exhaustive searching algorithm for 100 random shadow distributed PV arrays. For each PV array, 6 - 24 PV panels are connected into 3 - 6 PV strings. For each PV panel, it will has 3 - 8 working current values.
