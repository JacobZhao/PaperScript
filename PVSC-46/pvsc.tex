...
%PV array reconfiguration have been investigated for connection among PV modules and PV panels. 
%Since one PV panel is composed of several PV modules, a reconfiguration problem for PV panels has a constraint
%that a series connection of PV modules in the same PV panel have to be kept, and 
...
%and such a mismatched condition might accelerate heating and aging of PV cells and cause further damaging. To prevent PV cells from damaging, bypass diodes are usually placed in PV arrays and they are turned on in mismatched conditions. However, it causes a significant loss of power generation. 



%A PV array is composed of interconnected PV panels. 
...
\begin{figure}
    \centering
    \includegraphics[width=0.7\linewidth]{PVSC-46/fig/compare.png}
...
% \cite{nguyen2008adaptive,wang2014architecture,storey2013improved,storey2014optimized,udenze2018reconfiguration}. 
...
%The experimental results demonstrate the effectiveness of the proposed method where ...
...
% Section \ref{Sec7} shows the experimental results. 
...
In Fig.~\ref{fig:IV}, one PV panel has three PV modules with different I-V characteristics with MPPs at current and voltage pairs of around (5A,~20V), (3A,~20V), and (1.5A,~20A), respectively as shown in Fig.~\ref{fig:IV}(a-c). The PV panel has three MPPs at around (5A,~20V), (3A,~40V), and (1.5A,~60A) as shown in Fig.~\ref{fig:IV}(d). 
...
$Q = (Q_{1},Q_{2},\ldots ,Q_{s})$ for strings $1,2,\ldots s$ with $m$ active modules per string. For example, for a current requirement (2.5A, 2.5A) with 3 active modules is realized by assigning panel $P2$ to the first string and panels $P1$ and $P4$ to the second string as shown in Fig.~\ref{fig:feasible-configuration}. However, we could not find a feasible assignment for a current pair (3A, 2.5A) with 3 active modules.

%There are three peaks in power (maximum power points, MPPs) where voltage values are roughly multiples of a unit voltage. Current levels of MPPs are roughly maximum current levels of three PV modules in In Fig.\ref{fig:IV}(a-c). 
...
%We will consider that when selecting $P1$ for a current level 3A, we don't have any loss for a current level 2.5A but lose one module for 1.5A and 0.5A. The algorithm select panels so as to minimize such losses. 
...
%To evaluate the effectiveness of the proposed algorithm, we compare the results with an exhaustive search algorithm.
% for 100 random shadow distributed PV array. For each PV array, contains 
%6 - 24
